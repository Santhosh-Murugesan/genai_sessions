...
def find_similar_documents(query_embeddings, index, k=50):
...
def fetch_document_by_id(id):
    with open("document_metadata_from_csv.json", "r") as f:
...
def relevant_chunks(query):
    ...
    index = faiss.read_index("C:\\Users\\Santhosh\\OneDrive\\Documents\\GenAI_Workspace\\genai_sessions\\embeddings_with_vector_stores\\document_embeddings_from_csv.faiss")

    #print(index)
    
    #step 5: get the similarity scores of all the chunks in the database.
    distances, indices = find_similar_documents(query_embeddings, index, k=60) #tensor([[0.327940, 0.294255, 0.607475, 0.494079,........0.272311]])
